...
class MongoSequencesIntegrationTests {
	...
	void shouldGenerateSequentialValues() {
		...
		MongoSequences factory = new DefaultMongoSequences(template);
	...
	void shouldRespectCustomStartValue() {
		...
		MongoSequences factory = new DefaultMongoSequences(template);
		MongoSequence<Long> seq = factory.numericSequence("custom-seq", cfg -> cfg.startWith(1000L));
	...
	void shouldRespectCustomIncrement() {
		...
		MongoSequences factory = new DefaultMongoSequences(template);
		MongoSequence<Long> seq = factory.numericSequence("increment-seq", cfg -> cfg.startWith(10L).incrementBy(5L));
	...
	void shouldPersistAcrossFactoryInstances() {
		...
		MongoSequences factory1 = new DefaultMongoSequences(template);
		...
		MongoSequences factory2 = new DefaultMongoSequences(template);
	...
	void shouldUseCustomCollection() {
		...
		MongoSequences factory = new DefaultMongoSequences(template);
		MongoSequence<Long> seq = factory.numericSequence("test-seq", cfg -> cfg.collection(customCollection));
	...
	void shouldHandleHighConcurrencyWithCustomConfiguration() throws Exception {
		...
		for (int i = 0; i < threadCount; i++) {
			new Thread(() -> {
				MongoSequences factory = new DefaultMongoSequences(template);
				MongoSequence<Long> seq = factory.numericSequence("high-concurrency-seq",
						cfg -> cfg.startWith(100L).incrementBy(2L));
	...
	@Test // GH-4823
	void shouldUseMongoTemplateSequenceFactory() {

		MongoSequence<Long> seq = template.sequenceFactory().numericSequence("template-factory-seq");
